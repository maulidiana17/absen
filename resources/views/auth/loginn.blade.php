...
<head>
    ...
    <link rel="stylesheet" href="/assets/css/style_login.css" type="text/css" />
    <title>Login Form </title>
</head>
<body>
    <section>
        <section id="span-container"></section>
        <div class="signin">
            <div class="content">
                <h2>
                    Presensi Siswa
                ...
                <form action="/prosesLoginn" method="POST" class="form">
                    @php
                    ...
                    @endif
                    @csrf
                    <div class="inputBx inputBx1">
                        <label for="loginNis" id="loginNisLabel"></label>
                        <input type="text" name="nis" id="loginNis" maxlength="254" />
                        <i>Masukkan NIS </i>
                    </div>
                    <div class="inputBx inputBx2">
                        <label for="loginPassword" id="loginPasswordLabel"></label>
                        <input type="password" name="password" id="loginPassword" />
                        <i>Masukkan Password </i>
                        <label id="showPasswordToggle" for="showPasswordCheck"> Show Password
   ...
   <script>
    var textLabel = document.querySelector('#loginNisLabel'),
    text = document.querySelector('#loginNis'),
    ...
    var activeElement, curTextIndex, screenCenter,
    svgCoords, textCoords, textScrollMax,
    chinMin = .5, dFromC, mouthStatus = "small",
    blinking, eyeScale = 1, eyesCovered = false, showPasswordClicked = false;
    var eyeLCoords, eyeRCoords, noseCoords, mouthCoords,
    eyeLAngle, eyeLX, eyeLY, eyeRAngle, eyeRX, eyeRY, noseAngle,
    noseX, noseY, mouthAngle, mouthX, mouthY, mouthR, chinX,
    chinY, chinS, faceX, faceY, faceSkew, eyebrowSkew, outerEarX, outerEarY, hairX, hairS;

function calculateFaceMove(e) {
	var
		carPos = text.selectionEnd,
	...
	if(text.scrollWidth <= textScrollMax) {
		caretCoords = getPosition(span);
		dFromC = screenCenter - (caretCoords.x + textCoords.x);
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, textCoords.x + caretCoords.x, textCoords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, textCoords.x + caretCoords.x, textCoords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, textCoords.x + caretCoords.x, textCoords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, textCoords.x + caretCoords.x, textCoords.y + 25);
	} else {
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, textCoords.x + textScrollMax, textCoords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, textCoords.x + textScrollMax, textCoords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, textCoords.x + textScrollMax, textCoords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, textCoords.x + textScrollMax, textCoords.y + 25);
	}
...
function initLoginForm() {
	...
	textScrollMax = text.scrollWidth;
